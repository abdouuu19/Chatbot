...
// NEW: Group Chat Detection Helper
// IMPROVED Group Chat Detection Helper
class GroupChatHelper {
    // Check if the message is from a group chat
    static isGroupChat(webhookEvent) {
        // Multiple ways to detect group chats in Facebook Messenger
        return webhookEvent.message && (
            // Check thread_type if available
            webhookEvent.thread_type === 'GROUP' ||
            // Check if recipient is different from sender (group indicator)
            (webhookEvent.recipient && webhookEvent.recipient.id !== webhookEvent.sender.id) ||
            // Check message metadata for group indicators
            (webhookEvent.message.metadata && 
             webhookEvent.message.metadata.includes('GROUP')) ||
            // Check if there are multiple participants
            (webhookEvent.message.reply_to && webhookEvent.message.reply_to.mid)
        );
    }
    
    // Extract mentions from message - IMPROVED
    static extractMentions(messageText, attachments = null) {
        if (!messageText) return [];
        
        const mentions = [];
        
        // Look for @ChatwMe mentions (case insensitive, with variations)
        const mentionPatterns = [
            /@ChatwMe/gi,
            /@chatwme/gi,
            /@CHATWME/gi,
            /ChatwMe/gi // Sometimes @ symbol might be missing
        ];
        ...
        // Also check attachments for mentions if they exist
        if (attachments) {
            for (const attachment of attachments) {
                if (attachment.type === 'fallback' && attachment.title && 
                    attachment.title.toLowerCase().includes('chatwme')) {
                    mentions.push({
                        type: 'user',
                        name: 'ChatwMe',
                        position: 0,
    ...
    // Check if bot is mentioned - IMPROVED
    static isBotMentioned(messageText, attachments = null) {
        if (!messageText && !attachments) return false;
        
        const mentions = this.extractMentions(messageText, attachments);
        return mentions.length > 0;
    }
    
    // Clean message text by removing bot mentions - IMPROVED
    static cleanMentionsFromMessage(messageText) {
        if (!messageText) return '';
        
        // Remove all variations of ChatwMe mentions
        let cleanedMessage = messageText
            .replace(/@ChatwMe\s*/gi, '')
            .replace(/@chatwme\s*/gi, '')
            .replace(/@CHATWME\s*/gi, '')
            .replace(/ChatwMe\s*/gi, '') // Sometimes @ is missing
            .trim();
        
        return cleanedMessage;
    }
    
    // Get message without mention for processing
    static getQuestionFromMention(messageText) {
        const cleanedMessage = this.cleanMentionsFromMessage(messageText);
        return cleanedMessage;
    }
}
...
// NEW: Group chat greeting responses
const getGroupChatGreeting = (language) => {
    const responses = {
        english: {
...
// NEW: Enhanced message sending with reply-to-message functionality
const sendMessage = async (senderId, messageText, buttons = null, replyToMessageId = null) => {
    try {
        let messageData = {
        ...
        // Add reply-to functionality for group chats
        if (replyToMessageId) {
            messageData.message.reply_to = {
        ...
        if (buttons && buttons.length > 0) {
            ...
            if (messageText.length > 1000) { // Reduced from 2000
                ...
                for (let i = 0; i < chunks.length && i < 2; i++) { // Max 2 chunks instead of 3
                    await sendMessage(senderId, chunks[i].trim(), null, replyToMessageId);
...
// UPDATED: Main message processing function with group chat support
const processMessage = async (senderId, messageText, attachments = null, webhookEvent = null) => {
    try {
        // NEW: Check if this is a group chat
        const isGroupMessage = webhookEvent && (
            webhookEvent.thread_type === 'GROUP' ||
            (webhookEvent.message && webhookEvent.message.is_echo === false && 
             webhookEvent.recipient && webhookEvent.recipient.id !== senderId)
        );
        
        logger.info(`Processing message - Group: ${isGroupMessage}, Sender: ${senderId}`);
        
        // NEW: Group chat logic - only respond when mentioned
        if (isGroupMessage) {
            const isMentioned = GroupChatHelper.isBotMentioned(messageText, attachments);
            
            if (!isMentioned) {
                // Ignore messages in group chats where bot is not mentioned
                logger.info(`Ignoring group message without mention from ${senderId}`);
                return;
            }
            
            // Clean the message from mentions for processing
            messageText = GroupChatHelper.getQuestionFromMention(messageText);
            logger.info(`Group mention detected, cleaned message: "${messageText}"`);
        }
        ...
        logger.info(`Detected language: ${detectedLanguage} for user ${senderId} (new: ${isNewUser}, group: ${isGroupMessage})`);
        ...
        // NEW: Reply to the original message in group chats
        const replyToMessageId = isGroupMessage ? webhookEvent.message?.mid : null;
        await sendMessage(senderId, mediaResponse, null, replyToMessageId);
        return;
    }
    ...
    // NEW: Handle empty mentions in group chats (just @ChatwMe without question)
    if (isGroupMessage && messageText.trim() === '') {
        const greetingResponse = getGroupChatGreeting(detectedLanguage);
        await sendTypingIndicator(senderId, 'typing_off');
        const replyToMessageId = webhookEvent.message?.mid;
        await sendMessage(senderId, greetingResponse.text, null, replyToMessageId);
        return;
    }
    
    // IMPROVED Welcome logic - only for truly new users (NOT in group chats)
    if (!isGroupMessage && isNewUser && UserSessionManager.shouldShowWelcome(userProfile)) {
    ...
    // Smart returning user greeting (only for direct messages)
    if (!isGroupMessage && !isNewUser && UserSessionManager.isReturningUser(userProfile) && userProfile.messageCount === 1) {
    ...
    if (isAskingAboutCreator(messageText)) {
        ...
        await sendTypingIndicator(senderId, 'typing_off');
        const replyToMessageId = isGroupMessage ? webhookEvent.message?.mid : null;
        await sendMessage(senderId, creatorResponse.text, creatorResponse.buttons, replyToMessageId);
        return;
    }
    ...
    if (isAskingAboutProfile(messageText)) {
        ...
        await sendTypingIndicator(senderId, 'typing_off');
        const replyToMessageId = isGroupMessage ? webhookEvent.message?.mid : null;
        await sendMessage(senderId, profileResponse.text, profileResponse.buttons, replyToMessageId);
        return;
    }
    
    // REDUCED thinking delay for faster responses
    const thinkingDelay = Math.min(Math.max(messageText.length * 30, 800), 2500);
    ...
    // Send response with reply-to for group chats
    await sendTypingIndicator(senderId, 'typing_off');
    const replyToMessageId = isGroupMessage ? webhookEvent.message?.mid : null;
    await sendMessage(senderId, aiResponse, null, replyToMessageId);
    
    logger.info(`Successful interaction with user ${senderId} (${detectedLanguage}), Group: ${isGroupMessage}, Messages: ${conversation.length}`);
    ...
    const replyToMessageId = webhookEvent && GroupChatHelper.isGroupChat(webhookEvent) ? 
        webhookEvent.message?.mid : null;
    await sendMessage(senderId, errorResponses[language] || errorResponses.english, null, replyToMessageId);
}
};
            ...
            for (const entry of body.entry) {
                for (const webhookEvent of entry.messaging) {
                    ...
                    if (webhookEvent.message) {
                        ...
                        // UPDATED: Pass webhookEvent for group chat detection
                        setImmediate(() => {
                            processMessage(senderId, messageText || '', attachments, webhookEvent);
                        });
                    }
                        ...
                        setImmediate(() => {
                            processMessage(senderId, `${title}: ${payload}`, null, webhookEvent);
                        });
                    }
                }
            }
...
// FIXED cleanup - preserve user sessions longer
cron.schedule('0 */6 * * *', () => { // Every 6 hours instead of hourly
    ...
    let cleanedConversations = 0; // FIXED: was cleanedCount before
    
    for (const [userId, profile] of userProfiles.entries()) {
        if (profile.lastMessageTime && profile.lastMessageTime < cutoffTime) {
            conversationMemory.delete(userId);
            cleanedConversations++; // FIXED: was cleanedCount
        }
    }
