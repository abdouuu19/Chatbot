...
// Simplified Groq API integration - Arabic and English only
const callGroqAPI = async (messages, language, useAdvancedModel = true) => {
    // Only essential models for Arabic and English
    const models = {
        arabic: 'allam-2-7b',                    // Best for Arabic
        english: 'llama-3.3-70b-versatile',     // Best for English
    ...
    if (language && !['arabic', 'english', 'darija'].includes(language)) {
        return language === 'french' ? 
            "Veuillez utiliser l'arabe ou l'anglais uniquement. يرجى استخدام العربية أو الإنجليزية فقط." :
            "Please use Arabic or English only. يرجى استخدام العربية أو الإنجليزية فقط.";
    }
    
    // Select model based on detected language
    const selectedModel = detectedLang === 'arabic' ? models.arabic : models.english;
    
    // Model configurations for smart but concise responses
    const config = selectedModel === models.arabic ? {
        max_tokens: 200,        // Allow more tokens for smart responses
        temperature: 0.4,       // Balanced creativity and focus
        top_p: 0.7             // Allow some creativity for smart answers
    } : {
        max_tokens: 250,        // More tokens for English
        temperature: 0.5,       // Slightly more creative
        top_p: 0.8             // Better for nuanced responses
    };
    
    // Smart but concise system prompt
    const systemPrompt = detectedLang === 'arabic' ? 
        'أنت مساعد ذكي ومفيد. أجب بذكاء ووضوح في جملة أو جملتين قصيرتين فقط. لا تشرح تفكيرك، فقط أعط الجواب المفيد مباشرة.' :
        'You are a smart and helpful assistant. Answer intelligently and clearly in 1-2 short sentences only. Don\'t explain your thinking, just give the useful answer directly.';
    ...
    try {
        const response = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
            ...
            top_p: config.top_p,
            frequency_penalty: 0.3,    // Moderate penalty for repetition
            presence_penalty: 0.3,     // Moderate penalty for verbosity
            stream: false,
            stop: ["أولاً:", "ثانياً:", "بداية:", "Let me think", "First,", "Second,", "أفكر في", "I need to"]
        }, {
            headers: {
        ...
        if (selectedModel !== models.fallback) {
            ...
            try {
                const response = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
                    ...
                    messages: finalMessages,
                    max_tokens: 180,       // Balanced for smart fallback
                    temperature: 0.4,
                    top_p: 0.7,
