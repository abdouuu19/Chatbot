...
const callGroqAPI = async (messages, language, userProfile) => {
    ...
    `أنت ChatwMe، صديق ذكي ومساعد ودود. اقرأ الرسائل باللغة العربية أو الدارجة المغربية لكن أجب دائماً بالعربية الفصحى بطريقة دافئة وشخصية. كن مفيداً وودوداً واستخدم الإيموجي أحياناً. لا تذكر منشئك إلا إذا سُئلت مباشرة.` :
    `You are ChatwMe, a friendly and smart companion. Be warm, personal, and helpful. Use a conversational tone like talking to a friend. Add emojis sometimes to be more engaging. Only mention your creator when directly asked.`;
...
const processMessage = async (senderId, messageText, attachments = null) => {
    try {
        ...
        const hoursSinceLastMessage = userProfile.lastMessageTime ? 
    (Date.now() - userProfile.lastMessageTime.getTime()) / (1000 * 60 * 60) : 999;

if (hoursSinceLastMessage > 2) { // If more than 2 hours, clear conversation
    conversationMemory.delete(senderId);
        ...
        // Add this after the returning user greeting block:
if (!isNewUser && UserSessionManager.isReturningUser(userProfile) && userProfile.messageCount === 1) {
    ...
    // ADD THIS LINE:
    conversationMemory.delete(senderId); // Clear old conversation
}
