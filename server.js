...
// Privacy Policy route - serve your existing HTML file  
...
// Advanced Groq API integration with multiple models optimized for Arabic and multilingual support
const callGroqAPI = async (messages, language, useAdvancedModel = true) => {
    const models = {
        // Production models (current and reliable)
        advanced: 'llama-3.3-70b-versatile',        // Latest Meta model with 128K context - best for complex tasks
        fast: 'llama-3.1-8b-instant',              // Fast Meta model with 128K context - quick responses
        arabic: 'allam-2-7b',                       // SDAIA's Arabic-specialized model (Preview)
        creative: 'llama3-70b-8192',                // Stable creative model
        
        // Fallback models
        fallback: 'llama3-8b-8192',                 // Most stable basic model
        
        // Preview models for specific tasks
        reasoning: 'deepseek-r1-distill-llama-70b', // Advanced reasoning (Preview)
        multilingual: 'qwen-qwq-32b'                // Good multilingual support (Preview)
    };
    
    // Select model based on language and task complexity
    let selectedModel;
    if (language === 'arabic' || language === 'darija') {
        // For Arabic/Darija, prefer ALLaM first, then advanced multilingual models
        selectedModel = useAdvancedModel ? models.arabic : models.multilingual;
    } else {
        // For other languages, use standard model selection
        selectedModel = useAdvancedModel ? models.advanced : models.fast;
    }
    ...
    const getModelConfig = (model) => {
        const configs = {
            'allam-2-7b': {
                max_tokens: 300,            // Much shorter for ALLaM
                temperature: 0.5,           // More focused responses
                top_p: 0.7                  // Less creative, more direct
            },
            ...
            'deepseek-r1-distill-llama-70b': {
                max_tokens: 250,            // Very short for reasoning
                temperature: 0.4,           // Very focused
                top_p: 0.75
            },
            'qwen-qwq-32b': {
                max_tokens: 350,            // Short responses
                temperature: 0.6,
                top_p: 0.8
            }
        };
        ...
        return configs[model] || {
            max_tokens: 300,            // Default to short responses
            temperature: 0.6,
            top_p: 0.8
        };
    };
    ...
    // Add system prompt to prevent verbose thinking and enforce concise responses
    const enhancedMessages = [...messages];
    
    // Check if system message exists, if not add one
    if (!enhancedMessages.some(msg => msg.role === 'system')) {
        enhancedMessages.unshift({
            role: 'system',
            content: language === 'arabic' || language === 'darija' ? 
                'أجب مباشرة وبإيجاز. لا تكتب عملية التفكير أو التحليل. أجب فقط على السؤال بوضوح وبشكل مختصر.' :
                'Answer directly and concisely. Do not write your thinking process or analysis. Just answer the question clearly and briefly.'
        });
    }
    ...
    try {
        const response = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
            ...
            max_tokens: Math.min(config.max_tokens, 500), // Limit tokens for shorter responses
            temperature: config.temperature,
            top_p: config.top_p,
            frequency_penalty: 0.2, // Increase to reduce repetition
            presence_penalty: 0.2,  // Increase to reduce verbosity
            stream: false
        }, {
            headers: {
        ...
        // If Arabic model fails, try multilingual model
        if (selectedModel === models.arabic) {
            logger.info('Arabic model failed, trying multilingual model...');
            try {
                const response = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
                ...
                logger.error('Multilingual fallback also failed:', fallbackError.response?.data || fallbackError.message);
            }
        }
        
        // Final fallback responses with improved Arabic and Darija
        const fallbackResponses = {
            ...
            arabic: "أعتذر، ولكن أواجه بعض الصعوبات التقنية في الوقت الحالي. يرجى المحاولة مرة أخرى خلال ثوانٍ قليلة! عادة ما أكون أكثر استجابة من هذا. 🤖💫",
            ...
            darija: "سمحني، بصح راني نواجه شوية مشاكل تقنية دابا. عاود جرب بعد شوية ثواني! عادة نكون أكثر استجابة من هاكا. 🤖💫",
            // Additional Algerian Darija variants
            algerian: "سماح، والو راني عندي مشكل تقني شوية. جرب تاني من بعد، عادة نكون نجاوب مليح برك! 🤖💫"
        };
...
// Helper function to detect if content needs Arabic-specialized processing
const needsArabicModel = (messages, language) => {
    if (language === 'arabic' || language === 'darija' || language === 'algerian') {
        return true;
    }
    
    // Check if any message contains Arabic script
    const arabicRegex = /[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/;
    return messages.some(msg => arabicRegex.test(msg.content));
};

// Enhanced wrapper function with language detection
const callGroqAPIEnhanced = async (messages, language, useAdvancedModel = true) => {
    // Auto-detect if Arabic model is needed
    const shouldUseArabicModel = needsArabicModel(messages, language);
    
    if (shouldUseArabicModel && language !== 'arabic') {
        logger.info(`Arabic script detected, switching to Arabic-optimized model for language: ${language}`);
        language = 'arabic'; // Use Arabic model configuration
    }
    
    return await callGroqAPI(messages, language, useAdvancedModel);
};
